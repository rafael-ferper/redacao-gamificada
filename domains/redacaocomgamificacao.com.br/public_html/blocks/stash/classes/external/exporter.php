...
abstract class exporter {
    ...
    final public static function get_create_structure() {
        ...
        foreach ($properties as $property => $definition) {
            ...
            if ($property == 'contextid') {
                ...
                $returns[$property] = new external_value($definition['type'], $property, $required, $default,
                    $definition['null']);
     ...
     * @return external_single_structure
     */
    final protected static function get_read_structure_from_properties($properties, $required = VALUE_REQUIRED,
            $default = null) {
        $returns = array();
        foreach ($properties as $property => $definition) {
